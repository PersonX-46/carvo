...
const WorkerProfile: React.FC = () => {
  ...
  const [activeSection, setActiveSection] = useState<'profile' | 'skills' | 'preferences' | 'security'>('profile');
    ...
    const fetchProfile = async () => {
      ...
      setTimeout(() => {
        const mockProfile: WorkerProfile = {
          ...
          currentWorkload: 3,
          rating: 4.8,
          address: '123 Jalan Teknisi, Taman Perindustrian, 70450 Seremban, Negeri Sembilan',
          emergencyContact: '+60 13-456 7890 (Siti - Spouse)',
  ...
  const handleNotificationChange = (key: keyof WorkerProfile['notifications'], value: boolean) => {
    if (profile) {
      setProfile({
        ...profile,
        notifications: {
  ...
  const getEmploymentDuration = (hireDate: string) => {
      ...
      <div className="space-y-6">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            ...
            <p className="text-gray-400">Manage your profile and professional information</p>
          </div>
        </div>
              ...
              <div className="flex items-center space-x-4 mb-4">
                <div className="w-16 h-16 bg-gradient-to-r from-amber-500 to-amber-600 rounded-2xl flex items-center justify-center shadow-lg">
                  <span className="text-white font-bold text-xl">
                    {profile ? getInitials(profile.name) : 'W'}
                  </span>
                </div>
                <div>
                  <p className="text-white font-semibold">{profile?.name}</p>
                  <p className="text-gray-400 text-sm">{profile?.position}</p>
                  <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium mt-1 ${getStatusColor(profile?.status || 'active')}`}>
              ...
              <div className="space-y-3">
                <div className="bg-gray-700/30 rounded-xl p-3">
                  <p className="text-amber-400 text-sm font-medium">
                    🎉 {profile ? getEmploymentDuration(profile.hireDate) : '0 months'} with ChengService
                  </p>
                </div>
                <div className="bg-gray-700/30 rounded-xl p-3">
        ...
        <div className="lg:col-span-3">
            ...
            <div className="bg-gradient-to-br from-gray-800/80 to-gray-900/80 backdrop-blur-sm rounded-2xl p-6 border border-gray-700/50">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                  ...
                  <p className="text-white font-medium text-lg">{profile.position}</p>
                </div>
                ...
                <div>
                  ...
                  {isEditing ? (
                    <input
                      ...
                      placeholder="+60 XXX XXXX (Relationship)"
                      className="w-full bg-gray-700/50 border border-gray-600 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-amber-500"
                    />
                  ) : (
              ...
              {!isEditing && (
                <div className="mt-6 pt-6 border-t border-gray-700/50">
                  <h4 className="text-gray-400 text-sm mb-4">Employment Information</h4>
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                    <div>
                      <p className="text-gray-400 text-sm">Employee ID</p>
                      <p className="text-white font-medium">W{profile.id.toString().padStart(6, '0')}</p>
                    </div>
                    <div>
                      <p className="text-gray-400 text-sm">Hire Date</p>
                      <p className="text-white font-medium">{formatDate(profile.hireDate)}</p>
                    </div>
                    <div>
                      <p className="text-gray-400 text-sm">Employment Duration</p>
                      <p className="text-amber-400 font-medium">{getEmploymentDuration(profile.hireDate)}</p>
                    </div>
                    <div>
          ...
          {activeSection === 'skills' && profile && (
            <div className="bg-gradient-to-br from-gray-800/80 to-gray-900/80 backdrop-blur-sm rounded-2xl p-6 border border-gray-700/50">
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2 mb-4">
                    {profile.skills.map((skill, index) => (
                      <span
                        key={index}
                        className="bg-amber-500/20 text-amber-400 px-3 py-2 rounded-lg border border-amber-500/30 flex items-center space-x-2"
                      >
                        <span>{skill}</span>
                        <button
                  ...
                  <div className="flex space-x-2">
                    <input
                      ...
                      className="flex-1 bg-gray-700/50 border border-gray-600 rounded-xl px-4 py-2 text-white placeholder-gray-400 focus:outline-none focus:border-amber-500"
                    />
                    <button
                ...
                <div>
                  ...
                  <div className="space-y-2 mb-4">
                    {profile.certifications.map((cert, index) => (
                      <div
                        key={index}
                        className="bg-gray-700/30 border border-gray-600/30 rounded-xl p-3 flex justify-between items-center"
                      >
              ...
              {/* Specialization */}
              <div className="mt-8 pt-6 border-t border-gray-700/50">
                <h4 className="text-white font-semibold mb-4">Specialization Areas</h4>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                  {profile.specialization.map((area, index) => (
                    <div
                      key={index}
                      className="bg-gradient-to-r from-amber-500/20 to-amber-600/20 border border-amber-500/30 rounded-xl p-4 text-center"
                    >
                      <span className="text-amber-400 font-medium">{area}</span>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div className="space-y-6">
                <div>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="space-y-3">
                      <h5 className="text-gray-400 text-sm">Notification Settings</h5>
                      <label className="flex items-center space-x-3 cursor-pointer">
                        <input
                          type="checkbox"
                          checked={profile.notifications.newAssignments}
                          onChange={(e) => handleNotificationChange('newAssignments', e.target.checked)}
                          className="w-4 h-4 text-amber-500 bg-gray-700 border-gray-600 rounded focus:ring-amber-500 focus:ring-2"
                        />
                        <span className="text-white">New assignments and tasks</span>
                      </label>
                      
                      <label className="flex items-center space-x-3 cursor-pointer">
                        <input
                          type="checkbox"
                          checked={profile.notifications.scheduleChanges}
                          onChange={(e) => handleNotificationChange('scheduleChanges', e.target.checked)}
                          className="w-4 h-4 text-amber-500 bg-gray-700 border-gray-600 rounded focus:ring-amber-500 focus:ring-2"
                        />
                        <span className="text-white">Schedule changes and updates</span>
                      </label>
                      
                      <label className="flex items-center space-x-3 cursor-pointer">
                        <input
                          type="checkbox"
                          checked={profile.notifications.performanceUpdates}
                          onChange={(e) => handleNotificationChange('performanceUpdates', e.target.checked)}
                          className="w-4 h-4 text-amber-500 bg-gray-700 border-gray-600 rounded focus:ring-amber-500 focus:ring-2"
                        />
                        <span className="text-white">Performance and rating updates</span>
                      </label>
                      
                      <label className="flex items-center space-x-3 cursor-pointer">
                        <input
                          type="checkbox"
                          checked={profile.notifications.emergencyAlerts}
                          onChange={(e) => handleNotificationChange('emergencyAlerts', e.target.checked)}
                          className="w-4 h-4 text-amber-500 bg-gray-700 border-gray-600 rounded focus:ring-amber-500 focus:ring-2"
                        />
                        <span className="text-white">Emergency and urgent alerts</span>
                      </label>
                    </div>
                  </div>
                </div>
                ...
                <div className="pt-4 border-t border-gray-700/50">
                  <h4 className="text-white font-semibold mb-4">Work Preferences</h4>
                  <div className="bg-gray-700/30 rounded-xl p-4">
                    <p className="text-gray-300 text-sm">
                      Your current specialization areas and work preferences help us assign you the most suitable tasks.
                    </p>
                    <div className="flex flex-wrap gap-2 mt-3">
                      {profile.specialization.map((area, index) => (
                        <span key={index} className="bg-amber-500/20 text-amber-400 px-3 py-1 rounded-lg text-sm">
                          {area}
                        </span>
                      ))}
                    </div>
