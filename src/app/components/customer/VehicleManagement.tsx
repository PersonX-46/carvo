...
const VehicleManagement: React.FC = () => {
  ...
  const fetchVehicles = async () => {
    try {
      ...
      // Use the dedicated vehicles API endpoint instead of auth/me
  ...
  const fetchServiceHistory = async (vehicleId: number) => {
    try {
      const response = await fetch(
        `/api/customer/vehicles/${vehicleId}/services`
      );
      if (response.ok) {
        const result = await response.json();
  ...
  const getExpiryStatus = (dateString: string | null) => {
    ...
    if (daysUntil < 0)
      return { status: "Expired", color: "text-red-400 bg-red-500/20" };
    if (daysUntil <= 30)
      return { status: "Soon", color: "text-orange-400 bg-orange-500/20" };
          ...
          <div className="bg-gradient-to-br from-gray-800/80 to-gray-900/80 rounded-2xl p-4 border border-gray-700/50">
            ...
            <p className="text-lg font-bold text-amber-400">
              {selectedVehicle.nextService
                ? new Date(selectedVehicle.nextService).toLocaleDateString()
                : "Not scheduled"}
            </p>
          </div>
          <div className="bg-gradient-to-br from-gray-800/80 to-gray-900/80 rounded-2xl p-4 border border-gray-700/50">
          ...
          <div className="bg-gradient-to-br from-gray-800/80 to-gray-900/80 rounded-2xl border border-gray-700/50 p-6">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <p className="text-white font-medium">
                  {selectedVehicle.year || "Not set"}
                </p>
              </div>
              <div>
                <p className="text-gray-400 text-sm">Type</p>
                <p className="text-white font-medium">
                  {selectedVehicle.type || "Not specified"}
                </p>
              </div>
              <div>
              ...
              <div>
                ...
                <p className="text-white font-medium">
                  {selectedVehicle.engineCapacity || "Not specified"}
                </p>
              </div>
              <div>
          ...
          <div className="bg-gradient-to-br from-gray-800/80 to-gray-900/80 rounded-2xl border border-gray-700/50 p-6">
            ...
            <div className="space-y-4">
              <div className="flex justify-between items-center">
                ...
                <span
                  className={`px-3 py-1 rounded-full text-xs font-medium ${insuranceStatus.color}`}
                >
              ...
              <div className="flex justify-between items-center">
                ...
                <span
                  className={`px-3 py-1 rounded-full text-xs font-medium ${roadTaxStatus.color}`}
                >
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredVehicles.map((vehicle) => {
            ...
            const needsService =
              vehicle.nextService && getDaysUntil(vehicle.nextService) <= 30;
              ...
              >
                ...
                <div className="space-y-3 mb-4">
                  ...
                  <div className="flex justify-between text-sm">
                    ...
                    <span
                      className={`px-2 py-1 rounded-full text-xs ${insuranceStatus.color}`}
                    >
                      {insuranceStatus.status}
                    </span>
                  </div>
