...
const AdminLogin: React.FC = () => {
  ...
  const handleDemoLogin = () => {
    setEmail('admin@chengservice.com');
      ...
      <div className="relative z-10 w-full max-w-xs xs:max-w-sm sm:max-w-md mx-auto px-3 xs:px-4 sm:px-6 lg:px-8">
        ...
        <div className="bg-gray-900/80 backdrop-blur-xl rounded-xl sm:rounded-2xl border-2 border-amber-500/30 shadow-2xl shadow-amber-500/20 p-4 xs:p-5 sm:p-6 lg:p-8">
          ...
          <div className="text-center mb-6 sm:mb-8">
            <div className="flex justify-center mb-3 sm:mb-4">
              <div className="w-12 h-12 xs:w-14 xs:h-14 sm:w-16 sm:h-16 bg-gradient-to-r from-yellow-500 to-amber-600 rounded-lg xs:rounded-xl sm:rounded-2xl flex items-center justify-center shadow-lg">
                <span className="text-white font-bold text-xl xs:text-2xl sm:text-3xl">⚙️</span>
              </div>
            </div>
            <h1 className="text-xl xs:text-2xl sm:text-3xl font-bold text-white mb-2">
              Admin <span className="bg-gradient-to-r from-yellow-400 to-amber-500 bg-clip-text text-transparent">Login</span>
            </h1>
            <p className="text-gray-400 text-xs xs:text-sm">
              Administrative access to workshop management system
            </p>
          </div>

          {/* Error Message */}
          {error && (
            <div className="mb-4 p-3 bg-red-500/10 border border-red-500/20 rounded-lg text-red-400 text-xs xs:text-sm">
          ...
          <div className="mt-4 xs:mt-5 sm:mt-6 p-3 xs:p-4 bg-gray-800/30 rounded-lg xs:rounded-xl border border-amber-500/20">
            ...
            <div className="text-xs text-gray-300 space-y-1">
              <div className="break-words">
                <span className="text-amber-400">Email:</span> admin@chengservice.com
              </div>
              ...
              <button
                ...
                disabled={isLoading}
                className="mt-2 text-amber-400 hover:text-amber-300 text-xs underline disabled:opacity-50"
              >
          ...
          <div className="mt-4 xs:mt-5 sm:mt-6 text-center">
            <p className="text-gray-400 text-xs xs:text-sm mb-2">
              Need other access?
            </p>
            <div className="flex justify-center space-x-3">
              <Link 
                href="/customer/login" 
                className="text-xs xs:text-sm text-amber-400 hover:text-amber-300 transition-colors bg-gray-800/50 px-3 py-1 rounded-lg border border-amber-500/20"
              >
                👤 Customer
              </Link>
              <Link 
        ...
        <div className="mt-4 xs:mt-5 sm:mt-6 text-center">
          <div className="inline-flex items-center space-x-1 xs:space-x-2 bg-gray-800/30 backdrop-blur-sm rounded-full px-3 xs:px-4 py-1 xs:py-2 border border-amber-500/20">
            <span className="text-amber-400 text-xs xs:text-sm">⚙️</span>
            <span className="text-gray-300 text-xs xs:text-sm">Admin Portal - ChengService</span>
          </div>
        </div>
      </div>
